...
@registry.register_loss("visual_caption_ignore_loss")
class VisualCaptionIgnoreLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])

    def forward(self, sample_list, model_output):
        scores = model_output["mmt_scores"]
        ignore_mask = model_output["ignore_mask"].float()  # [N, 30]
        targets = sample_list["targets"][:,:,:-50]
        loss_mask = sample_list["train_loss_mask"]
        assert scores.dim() == 3 and loss_mask.dim() == 2
        ...
        losses *= loss_mask.unsqueeze(-1)
        losses *= (1 - ignore_mask).unsqueeze(-1)  # remove the OCR token

        count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
        loss = torch.sum(losses) / count
        return loss

@registry.register_loss("softmax_anchor_pred_loss")
class SoftmaxAnchorPredLoss(nn.Module):
    def __init__(self):
        super().__init__()

    def forward(self, sample_list, model_output):
        gt_anchor = sample_list['anchor_score']  # [B, M]
        anchor_pred = model_output["anchor_pred"]  # [B, M]
        loss_mask = model_output["ocr_mask"]        # [B, M]

        anchor_loss = - (gt_anchor * F.log_softmax(anchor_pred, dim=-1))
...
@registry.register_loss("anchor_pred_loss")
class AnchorPredLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        ...
        scores = model_output["anchor_pred"]
        targets = sample_list["anchor_score"]
...
@registry.register_loss("graph_pred_loss")
class GraphPredLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
        loss_mask = model_output["ocr_mask"]
        scores = model_output["graph_pred"] + 0.5  
