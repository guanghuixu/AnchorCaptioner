...
from pythia.modules.refine_mmt import Refine_MMT
from pythia.modules.gpn import GraphProposalNetwork
...
class M4C(BaseModel):
    ...
    def _build_anchor_graph(self):
        self.graph_proposal_net = GraphProposalNetwork(self.graph_config)

    def _build_mmt(self):
        self.mmt = MMT(self.mmt_config)
        self.r_mmt = Refine_MMT(self.mmt_config)
    ...
    def forward(self, sample_list):
        ...
        self._forward_anchor_graph(sample_list, fwd_results)
        self._forward_mmt_and_output(sample_list, fwd_results)

        # only keep scores in the forward pass results
        results = {
            "scores": fwd_results["scores"],
            "mmt_scores": fwd_results["mmt_scores"],
            "ignore_mask": fwd_results["ignore_mask"],
            "anchor_pred": fwd_results["anchor_pred"],
            "graph_pred": fwd_results["graph_pred"],
    ...
    def  _forward_anchor_graph(self, sample_list, fwd_results):
        visual_emb, text_emb, anchor_scores, graph_scores, anchor = self.graph_proposal_net(
            text_emb=fwd_results['ocr_mmt_in'], text_mask=fwd_results['ocr_mask'], 
            visual_emb=fwd_results['obj_mmt_in'], visual_mask=fwd_results['obj_mask'],
            # cover_map=sample_list.cover_maps, 
        )
        # fwd_results['graph_obj'] = visual_emb * fwd_results['obj_mask'].unsqueeze(2)
        # fwd_results['graph_ocr'] = text_emb * fwd_results['ocr_mask'].unsqueeze(2)
        fwd_results['anchor'] = anchor
        fwd_results['anchor_pred'] = anchor_scores
        fwd_results['graph_pred'] = graph_scores * fwd_results['ocr_mask']
    ...
    def _forward_mmt(self, sample_list, fwd_results):
        ...
        mmt_visual_output, mmt_dec_output = self.mmt(
            obj_emb=fwd_results['obj_mmt_in'],
            obj_mask=fwd_results['obj_mask'],
            ocr_emb=fwd_results['ocr_mmt_in'],
            ocr_mask=(fwd_results['graph_pred'] > 0).float(),
            fixed_ans_emb=self.classifier.module.weight,
            prev_inds=fwd_results['prev_inds'],
        )
        fwd_results['mmt_dec_output'] = mmt_dec_output
        fwd_results['ignore_mask'] = fwd_results['prev_inds'] >= self.vocab_size 

        r_mmt_ocr_output, r_mmt_dec_output = self.r_mmt(
            visual_emb=mmt_visual_output,
            visual_mask=fwd_results['obj_mask'],
            anchor_emb=fwd_results['anchor'],
            ocr_emb=fwd_results['ocr_mmt_in'], 
            ocr_mask=(fwd_results['graph_pred'] > 0).float(),
            dec_emb=mmt_dec_output)
        fwd_results['r_mmt_ocr_output'] = r_mmt_ocr_output
        fwd_results['r_mmt_dec_output'] = r_mmt_dec_output

    def _forward_output(self, sample_list, fwd_results):
        # the output of mmt
        mmt_dec_output = fwd_results['mmt_dec_output']
        mmt_scores = self.classifier(mmt_dec_output)
        fwd_results['mmt_scores'] = mmt_scores

        # the output of refine_mmt
        r_mmt_ocr_output = fwd_results['r_mmt_ocr_output']
        r_mmt_dec_output = fwd_results['r_mmt_dec_output']
        ocr_mask = fwd_results['ocr_mask']

        fixed_scores = self.classifier(r_mmt_dec_output)
        dynamic_ocr_scores = self.ocr_ptr_net(
            r_mmt_dec_output, r_mmt_ocr_output, ocr_mask
        )
...
class MMT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        mmt_visual_output = mmt_seq_output[:, :obj_mask.size(1)]
        mmt_dec_output = mmt_seq_output[:, -dec_max_num:]

        return mmt_visual_output, mmt_dec_output
