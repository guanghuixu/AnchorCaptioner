...
class GenerateWord:
    def __init__(self, fixed_ans_path):
...
class BaseTrainer:
    ...
    def load(self):
        ...
        if visualization_flag:
            self.generator = GenerateWord('data/m4c_captioner_vocabs/textcaps/vocab_textcap_threshold_10.txt')
    ...
    def _summarize_visualization(self, report):
        # annotations
        question_ids = report['question_id']
        ocr_tokens = report['context_tokens']
        loss_mask = report['ocr_mask'].sum(dim=1)
        # predictions
        scores = report['scores'].argmax(dim=-1)
        mmt_scores = report['mmt_scores'].argmax(dim=-1)
        for i, question_id in enumerate(question_ids):
            val_predictions.append({
                'question_id': question_id.item(),
                'image_id': report['image_id'][i],
                'ocr': ' '.join(ocr_tokens[i]).replace('<pad>', '').strip(),
                'ACG_gt': report['anchor_score'][i].data.cpu().numpy(),
                'anchor_pred': report['anchor_pred'][i,:int(loss_mask[i].item())].data.cpu().numpy(),
                'graph_pred': report['graph_pred'][i,:int(loss_mask[i].item())].data.cpu().numpy()+0.5,
                'pred_t': ' '.join([self.generator.idx2word(pred, ocr_tokens[i]) for pred in scores[i]]).replace('</s>', '').strip(),
                'pred_v': ' '.join([self.generator.idx2word(pred, ocr_tokens[i]) for pred in mmt_scores[i]]).replace('</s>', '').strip()
            })
    ...
    def _inference_run(self, dataset_type):
        ...
        report, meter = self.evaluate(
            getattr(self, "{}_loader".format(dataset_type)), use_tqdm=False # True
        )
